...
## 2026-07-02 (4) 海底探検をプレイ可能に（G1・EditMode 37/37・実機未検証）

**方針（ユーザー承認済み）**: B案=物理エミュ+摩擦低減。ルール裁定はコード化しない（間違い→手役のジェスチャー訂正が観察対象）。C案（ターン管理・自動減算）は却下。
- 全ピース掴める（チップ21+空気マーカー 1.6倍スケール・グリッド間隔0.085）
- サイコロ2個・**確定表示方式**（[DiceRoller](../../Assets/TableDuo/Scripts/Net/DiceRoller.cs)）: 離すとサーバが1–3確定→ダイス上方に数字（NetworkVariable同期・保持中非表示）→CSV `dice` 行。物理投擲は体験が物足りなければ後日
- **盤面リセット**: `curl "http://<host>:7780/mark?label=reset_board"` → [BoardReset](../../Assets/TableDuo/Scripts/Net/BoardReset.cs) が初期配置へ復元（保持中は ServerForceRelease）
- protocol Phase4 を「協調配置」→「海底探検1ラウンド」に置換（PatternPanel/目標配置は残置・未使用化）
