...
- **MCP-free の検証は [[table_duo_l0_desktop_test]]（L0 Standalone build を batchmode で焼き CLI 起動）が最強**。検証で繰り返し詰まった真因は「実機が無いこと」ではなく**この MCP wedge**だった。Editor 単体検証（host+spectator 等）すら実機ゼロでできるので、本当の壁は MCP の安定性。batchmode ビルド＋`-logFile`＋ビルド内 `ScreenCapture` 自動保存なら Editor も MCP も介さず Bash/PowerShell だけで完結する。**`computer-use` は Builds/ の未登録 standalone exe を名前解決できない**（Start メニュー登録アプリのみ）ので、観戦/キービジュアルの実画像はビルド内の自動スクショ→`Read` で取る
- **PowerShell ツールの注意**: `Remove-Item` はサンドボックスがパス（"C:\Program" 等）を誤判定して**ブロックされることがある**。ファイル削除は `[System.IO.File]::Delete($path)` を使う（try/catch で囲む）
