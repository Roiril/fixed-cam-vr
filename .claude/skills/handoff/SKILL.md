---
name: handoff
description: 次セッションへの引き継ぎプロンプトを生成。ユーザーが「セッション終わり / 引き継ぎ作って / 中断する」等を示唆した時に呼ぶ。git 状態 + Unity MCP 状態 + 直近完了タスクを構造化し、コピペで次セッションを即再開できる形で出力する。
---

# 引き継ぎプロンプト生成

セッション終了時、次セッション開始時にユーザーが貼ることで作業を即再開できる引き継ぎプロンプトを生成する。
...
- 引き継ぎプロンプト本文は **メモリ保存しない**（エフェメラル情報）。プロジェクト固有の永続的な事実だけ memory に分離保存
