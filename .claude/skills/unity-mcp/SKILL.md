---
name: unity-mcp
description: Unity MCP (MCP for Unity) の接続診断と再接続。`mcp__UnityMCP__*` ツールが見えない / 呼び出しが失敗する / ブリッジ未起動が疑われる時に呼ぶ。`unity-status` でツール不在を確認した時の次手。再起動・再登録など副作用ある操作はユーザ合意を取る。
---

# Unity MCP 接続診断

`unity-status` を呼んでも `mcp__UnityMCP__*` が見えない / `connect refused` が出る等、MCP 接続そのものが疑われる時に走らせる。**順に** 実行し、最後にユーザへ「次に何をすればいいか」を 1 つだけ提示する。
...
- Claude Code 側のサーバ登録は **ユーザースコープ**（`~/.claude.json` の トップレベル `mcpServers`）に置くのが安定
  - プロジェクトスコープは `cwd` 表記揺れで不可視化することがあるため避ける
...
ヒット時は `manage_editor action=get_state` と `mcpforunity://instances` を読み、接続健全性を確認する。

旧名 `mcp__unityMCP__*`（小文字 u）が出てくる古い登録に当たった場合は、ユーザに削除と再登録を提案する。
...
判定：
- `claude mcp list` に `UnityMCP` 系が無い → **登録欠落**（プロジェクトスコープに埋もれている可能性も含む — `~/.claude.json` の `projects.*.mcpServers` を python で grep）
...
Editor 内で `Window → MCP for Unity → Status` を開いて bridge を Start するようユーザに案内。
...
**第一選択**: Unity Editor 内の **`Window → MCP for Unity → Auto Setup`**。
...
- `-s user` でユーザースコープ
- バージョンは `~/.local/bin/uvx.exe` に install 済みのものに合わせる
- `--offline` で git fetch を回避（オンライン取得は本環境で失敗実績あり）
- 命名は **大文字 `UnityMCP`**（権限 allow リストと揃える）

**避けるべき形式**:
- ❌ `git+https://...` 形式 — git fetch 失敗
- ❌ `claude mcp add-json` で `"type":"stdio"` — `Invalid input` で弾かれる
- ❌ プロジェクトスコープ — cwd キー揺れで `claude mcp list` から消える

## 6. 補足ルール

- **副作用ある操作は必ずユーザ合意**: `claude mcp remove`, `claude mcp add`, Editor 再起動依頼
- 同じ診断を 1 セッション内で 2 回繰り返すなら、メモリ追記を提案
