...
- **映像入力**: スマホ複数台 (DroidCam / IP Webcam) → MJPEG over Wi-Fi → Unity 内デコード
...
- **[settings.json](.claude/settings.json)** — プロジェクト固有の権限・hook（必要時のみ作成）
...
- [streaming.md](.agent/rules/streaming.md) — MJPEG / WebRTC ストリーミング取り込み
