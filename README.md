...
| 2.7 | プレイヤー位置連動カメラ切替（[Assets/Scripts/Tracking/](Assets/Scripts/Tracking/)）+ HMD 内ゾーン校正（ZoneCalibrator）+ PlayerZone 検証シーン | ✅ 実装完了・実機でゾーン校正運用中 |
| 3 | パススルー暗転 + CCTV 風シェーダ + CG 合成手法の比較（[Assets/Scripts/Fx/](Assets/Scripts/Fx/)）| ✅ 4 系統プロトタイプ完了・本実装着手前 |
| 3.5 | 映像差し替え（OverlayCue）+ Web オペレータ卓からの遠隔制御（ShowControlClient / [tools/web-compositor/](tools/web-compositor/)）| ✅ 実装済み・運用検証中 |
...
### Diagnostics（起動保護・デバッグ・検証支援）

[Assets/Scripts/Diagnostics/](Assets/Scripts/Diagnostics/)（`FixedCamVr.Diagnostics`）

- `StartupFader` — Play 直後の OVR 初期化 / 砂時計 / ストリーム接続待ちを head-lock 黒 Canvas で覆い、安定フレーム到達後にフェードアウト（目の保護）
- `RuntimeDebugHud` — FPS / 受信状態 / 現在ゾーン等を World Space HUD に表示。`HudToggleInput`（Y or H キー）でトグル
- `HudLogDumper` — HUD と同値を `[HudDump]` プレフィックスで Console へ（MCP `read_console` で時系列取得）
- `HmdTrajectoryRecorder` — 実機で HMD 位置 / ゾーン含有を CSV 出力（`adb pull` でゾーン配置の妥当性を解析）

### 映像差し替え / オペレータ卓連携

- [ScreenOverlayController.cs](Assets/Scripts/Streaming/ScreenOverlayController.cs) + [OverlayCue.cs](Assets/Scripts/Streaming/OverlayCue.cs)（`Create > FixedCamVr > Overlay Cue`）— 事前撮影クリップ/静止画/URL + マスクをスクリーン枠空間で合成。キーボード / `PlayCue()` / Web の演出 ON/OFF で発火。フェード・再生区間(trim)・再生終了で自動復帰に対応。動画 URL はローカル DL してから再生（Android の HTTP ストリーミング相性問題回避）（[.claude/rules/streaming.md](.claude/rules/streaming.md)）
- [ShowControlClient.cs](Assets/Scripts/Streaming/ShowControlClient.cs) — Web オペレータ卓（`tools/web-compositor/`、1 ページ縦割り UI）から long-poll で Unity を遠隔制御（演出 ON/OFF・カメラ切替・ポスト FX）。さらに `show.json` を**設定契約**として、カメラ IP / 認証・**カメラ別画像加工**を実機が参照し、`persistentDataPath` にキャッシュして **PC 不在でも前回設定で起動**する（焼き込み.asset < 端末キャッシュ < ライブの後勝ち）。詳細 [.claude/rules/streaming.md](.claude/rules/streaming.md)「show.json = 設定契約」/ [plans/2026-06-16](.claude/plans/2026-06-16_web-config-to-quest.md)
...
| [docs/ivrc-video/](docs/ivrc-video/) | IVRC2026 ビデオ審査の制作プラン（構成・絵コンテ・ナレーション台本・音楽SFX選定・撮影リスト・AI 予告プロンプト・原理図/エンドカード素材） |
| [tools/web-compositor/README.md](tools/web-compositor/README.md) | Web オペレータ卓 + 映像合成検証ツール（起動・パイプライン・サーバ API） |
...
│   │   └── FxSandbox.unity             # FX 比較検証（Setup FxSandbox Scene で再生成可・Editor 専用）
│   ├── Scripts/
│   │   ├── Streaming/        # MJPEG 受信・描画 + オーバーレイ/オペレータ卓連携（Phase 1 / 2 / 2.5 / 3）
│   │   ├── OvrBridge/        # OVRInput ↔ asmdef 境界ブリッジ（Assembly-CSharp）
│   │   ├── Tracking/         # プレイヤー位置連動カメラ切替 + ZoneCalibrator（Phase 2.7）
│   │   ├── Diagnostics/      # StartupFader / HUD / HMD 軌跡記録
│   │   └── Fx/               # 映像加工 / CG 合成プロトタイプ（Phase 3）
│   ├── TableDuo/             # 同居サブプロジェクト：テーブル2人非対称VR 調査（下記セクション参照）
│   └── Settings/             # URP / Quality / CameraSource SO
├── docs/                     # 設計・現場・研究ドキュメント（onsite-checklist / table-duo/ 等）
├── tools/                    # web-compositor（オペレータ卓 + 映像合成検証）/ ログ取得スクリプト
├── Packages/manifest.json    # Meta XR SDK / Netcode for GameObjects 等の依存
