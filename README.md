...
- `ZoneCalibrator` — HMD 内で Quest コントローラだけでゾーン（中心・サイズ）を実地調整。両グリップ 1 秒長押しで校正モード ON/OFF、左スティック=中心移動、右スティック=サイズ伸縮、X=保存（`persistentDataPath/zone_calibration.json`）/ Y=リセット。起動位置をコース中心に合わせる recenter 付き
...
- [ScreenOverlayController.cs](Assets/Scripts/Streaming/ScreenOverlayController.cs) + [OverlayCue.cs](Assets/Scripts/Streaming/OverlayCue.cs)（`Create > FixedCamVr > Overlay Cue`）— 事前撮影クリップ/静止画 + マスクをスクリーン枠空間で合成。キーボード or `PlayCue()` で発火（[.claude/rules/streaming.md](.claude/rules/streaming.md)）
- [ShowControlClient.cs](Assets/Scripts/Streaming/ShowControlClient.cs) — Web オペレータ卓（`tools/web-compositor/` コンソールタブ）から long-poll で Unity を遠隔制御（cue 発火・カメラ固定・ポスト FX）。詳細 [.claude/memory/web_compositor.md](.claude/memory/web_compositor.md)
...
| **両グリップ** 1 秒長押し | ゾーン校正モード ON/OFF（[ZoneCalibrator](Assets/Scripts/Tracking/ZoneCalibrator.cs)・校正中は左右スティックで中心/サイズ調整・X 保存 / Y リセット） | `OvrControllerBridge` → `ZoneCalibrator.Feed()` |
