...
```
ステータス  … Unity 生存 / アクティブカメラ / fps / 反映 rev / server / 🚶ゾーン自律 / 📂撮影フォルダ
境界ブレンド … フェザー / 色統計 ON+強度 / ラプラシアン ON+レベル（全カメラ共通）
マルチカメラ（列 = カメラ A / B / C）
  ビュー      … 画質+合成を当てた最終見た目（Quest と同じ絵）
                + 📺切替（Quest 表示を切替）/ 演出 ON/OFF トグル + fade秒 / 📷 生1枚 / ⏺ 生録画（→ recordings/）
  設定        … IP / port / 認証 ＋ カメラ別 画質（露出・コントラスト・彩度・色温度・ヴィネット・グレイン・走査線）
  マスク      … 白黒の境界をスライダ調整。白の向き（左/右/上/下）＋位置スライダ＋取り消し（白=差し替え）
  合成素材    … captures/ から画像 or 動画を選択 / ↻更新 / 📂フォルダ / 動画は再生区間(trim) → 💾 cue 保存
下部: 生成プロンプト … 画像/動画 AI 生成プロンプトを 📋コピー / 編集 / 削除（prompts.json）
```

ビューはスライダを動かすと**その場で反映**（画質も境界ブレンドも見える）。📷/⏺ は**生フレーム**（加工・合成なし）を保存する。
...
| POST | `/save?type=image\|video&to=recordings&cam=A` | 📷 静止画 / ⏺ 録画の保存（`to=recordings` で撮影フォルダ、`cam` でファイル名接頭辞） |
